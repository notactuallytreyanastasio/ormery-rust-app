...
pub (crate) fn init() -> temper_core::Result<()> {
    ...
    INIT_ONCE.get_or_init(| |{
            ...
            let return__1000: temper_core::List<i32> = buildEscapeNeeds__163();
            ESCAPE_NEEDS.set(return__1000.clone()).unwrap_or_else(| _ | panic!());
            let return__1001: RegexRefs = RegexRefs::new(None, None, None, None);
            REGEX_REFS.set(return__1001.clone()).unwrap_or_else(| _ | panic!());
            Ok(())
    }).clone()
}
...
fn buildEscapeNeeds__163() -> temper_core::List<i32> {
    ...
    'loop___2678: while Some(code__382) <= Some(127) {
